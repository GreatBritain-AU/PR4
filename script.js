...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const questions = [
    { question: "Якого кольору бургер?", answers: [{ title: 'Стандарт', url: './image/burger.png' }, { title: 'Чорний', url: './image/burgerBlack.png' }], type: 'radio' },
    { question: "З якого м'яса котлета?", answers: [{ title: 'Курка', url: './image/chickenMeat.png' }, { title: 'Яловичина', url: './image/beefMeat.png' }, { title: 'Свинина', url: './image/porkMeat.png' }], type: 'radio' },
    { question: "Додаткові інгредієнти?", answers: [{ title: 'Помідор', url: './image/tomato.png' }, { title: 'Огірок', url: './image/cucumber.png' }, { title: 'Салат', url: './image/salad.png' }, { title: 'Цибуля', url: './image/onion.png' }], type: 'checkbox' },
    { question: "Додати соус?", answers: [{ title: 'Часниковий', url: './image/sauce1.png' }, { title: 'Томатний', url: './image/sauce2.png' }, { title: 'Гірчичний', url: './image/sauce3.png' }], type: 'radio' }
  ];
  ...
  const createPhoneUI = () => {
    ...
    phoneContainer.innerHTML = `
      ...
      <div style="display:flex;gap:8px;align-items:center">
        <input id="phoneInput" type="tel" inputmode="numeric" pattern="[0-9+]*" placeholder="+380XXXXXXXXX" style="padding:6px;flex:1">
  ...
  btnOpenModal.addEventListener('click', () => {
    ...
    if (!isTestInitialized) { initTest(); isTestInitialized = true; }
    renderQuestions(0);
  });
    ...
    questions[index].answers.forEach((answer, i) => {
      ...
      answerItem.innerHTML = `
        <div class="answers-item d-flex flex-column">
          <input type="${questions[index].type}" id="${inputId}" name="${inputName}" class="d-none" ${isChecked ? 'checked' : ''}>
  ...
  function initTest() {
    ...
    // --- SEND -> push to Firebase Realtime DB ---
    sendButton.addEventListener('click', async () => {
      const phoneInput = document.querySelector('#phoneInput');
      const phoneVal = phoneInput ? phoneInput.value.trim() : '';
      if (!phoneVal) { alert('Введіть номер телефону.'); return; }
      order.phone = phoneVal;

      // Подготовим удобный объект: вместо индексных ключей преобразуем в объект по вопросам
      const friendlyAnswers = {};
      questions.forEach((q, idx) => {
        friendlyAnswers[q.question] = order.answers[idx] ?? (q.type === 'checkbox' ? [] : '');
      });
      ...
      try {
        // push в коллекцию orders
        const ordersRef = ref(db, 'orders');
        const newOrderRef = push(ordersRef); // создаём новый ключ
        await set(newOrderRef, finalOrder);

        console.log('Заказ записан в БД:', finalOrder);
  ...
  // getData(path) возвращает Promise c данными (одноразово)
